 ...
 */
export async function uploadAndInstallDeployScript(session:SshSession, user:string){
    ...
    const remoteDeployBasePath = path.posix.join(await session.home(user), appsContainer);
    ...
    for (const file of files) {
        const _f = path.posix.join(remoteTempDir, file);
        const localFilePath = path.join(__dirname, '../_instructions/', file);
        const remoteFilePath = path.posix.join(remoteDeployBasePath,file);
