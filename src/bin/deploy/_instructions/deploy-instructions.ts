 ...
 * @returns 
 */
function loadPm2EcosystemFile(pm2File: string): { apps: GenericObject[] }{
 ...
 * @returns
 */
async function readStatus (projectName:string) {
    const status = await spawn(pm2, ['jlist'], { cwd: appsContainer, silent: true });
    const statusJson: GenericObject[] = JSON.parse(status.data.toString().trim());
    let error = null;
...
async function lsBackups () {
    const scanDirRecoursive = function (dirPath:string, arrayOfFiles:{ path:string, size: string, ctime:Date }[]) {
 ...
 * 
 */
async function cleanBackups(projectName?: string){
    const files = await lsBackups();

    const projects: { [k:string]: { ctime: Date,filename:string }[] } = {};
    files.forEach(file => {
...
async function deploy() {
    const projectName = argvParam('-p') as string;
...
async function restoreBackup () {
    ...
    const _opt = {
        ...
        strict: true,
        preservePaths: true,
        onwarn: (message: string, data: Buffer) => {
...
*/
function parseDockerJsonOutput(data:Buffer|string) {
