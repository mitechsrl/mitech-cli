...
# setup repo & install
curl -fsSL https://www.mongodb.org/static/pgp/server-6.0.asc | sudo gpg --batch --no-tty --dearmor -o /etc/apt/trusted.gpg.d/mongodb.gpg
...
HAVESSL=`cat /etc/mongod.conf | grep requireTLS |  wc -l`
if [[ $HAVESSL -eq 0 ]]
then
# add ssl config
sed -i "s/^net\:$/net:\n  tls:\n    mode: requireTLS\n    certificateKeyFile: \/etc\/mongodb\/mongodb.pem/" /etc/mongod.conf
fi
...
mongosh --tls --tlsAllowInvalidCertificates < /tmp/mongodbUsers.js
